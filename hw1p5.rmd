...
Among equally-spaced 2, 3, ..., and 20 knots, 15 knots gave us the least LOOCV/GCV value.
...
First of all, let me use 'smooth.spline' function in 'splines' package.
...
LOOCV fit(blue) and GCV fit(red) are almost the same.
Sadly, smooth.spline function does not offer standard error estimate.

To get the se, I will use 'mgcv' package.
The 'gam' function seems that it automatically choose the number of spline using GCV.
...
The above plot shows fitted curve and its 95% interval.
The 'gam' function gives us different fitted result from smooth.splines, though.
...
Last part's dataframe output is for prediction interval at $x=95$, and $x=115$.
I think that the interval lengths is too narrow, as well.

### 5-d. A local regression fit

I will use 'loess' function in the base-R.
Using GCV, the 'span' argument (smoothing parameter) in 'loess' function will be selected.

Let me skip using 'LOOCV' here, cause the linear smoother's one term, which is weight matrix(tri-cube function's or gaussian's), depends on each $x$ value.
As a result, it requires too many calculations to get $S_{ii}$, which is needed to get LOOCV.
